    ...
    public class StockAdapter : BaseAdapter
    {
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            if (view == null) // no view to re-use, create new
            {
                ...
                TextView colourView = view.FindViewById<TextView>(Resource.Id.textView5);    //color
                TextView sizeView = view.FindViewById<TextView>(Resource.Id.textView4);    //size
                EditText nzStockView = view.FindViewById<EditText>(Resource.Id.editText1);    //nzstock
                EditText ausStockView = view.FindViewById<EditText>(Resource.Id.editText2);    //austock

                String img = ImageName;

                colourView.Text = Colour[position];
                sizeView.Text = Size[position];

                nzStockView.Text = tshirtEntity[position].StockNZ.ToString();
                ausStockView.Text = tshirtEntity[position].StockAU.ToString();
